...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    DXUTCreateWindow( L"PCF With Varying Penumbra Size" );
...
void InitApp()
{
	...
	g_HUD.AddCheckBox( IDC_ENABLE_PCSS, L"Enable PCSS", 0, iY += iYo, 170, 22, g_bEnablePCSS );
...
void CALLBACK OnFrameMove( double fTime, float fElapsedTime, void* pUserContext )
{
	...
	g_LightManager.EnablePCSS(g_bEnablePCSS);
}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
		...
		case IDC_CAST_SHADOW:
			...
			break;
		case IDC_ENABLE_PCSS:
			g_bEnablePCSS = ((CDXUTCheckBox*)pControl)->GetChecked();
			break;
    }
}
