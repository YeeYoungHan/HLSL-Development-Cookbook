...
CLightManager::CLightManager() : m_bEnablePCSS(true), m_iLastShadowLight(-1), m_iNextFreeSpotShadowmap(-1),
	...
	m_pSpotLightVertexShader(NULL), m_pSpotLightHullShader(NULL), m_pSpotLightDomainShader(NULL), m_pSpotLightPixelShader(NULL), m_pSpotLightShadowPixelShader(NULL), m_pSpotLightPCSShadowPixelShader(NULL),
...
HRESULT CLightManager::Init()
{
	...
	V_RETURN( CompileShader(str, NULL, "SpotLightShadowPCSSPS", "ps_5_0", dwShaderFlags, &pShaderBlob) );
	V_RETURN( g_pDevice->CreatePixelShader( pShaderBlob->GetBufferPointer(),
		pShaderBlob->GetBufferSize(), NULL, &m_pSpotLightPCSShadowPixelShader ) );
	DXUT_SetDebugName( m_pSpotLightPCSShadowPixelShader, "Spot Light PCSShadow PS" );
	SAFE_RELEASE( pShaderBlob );
...
void CLightManager::Deinit()
{
	...
	SAFE_RELEASE( m_pSpotLightPCSShadowPixelShader );
...
void CLightManager::SpotLight(ID3D11DeviceContext* pd3dImmediateContext, const D3DXVECTOR3& vPos, const D3DXVECTOR3& vDir, float fRange, float fInnerAngle, float fOuterAngle, const D3DXVECTOR3& vColor, int iShadowmapIdx, bool bWireframe)
{
	...
	if(!bWireframe)
	{
		...
		if(iShadowmapIdx >= 0)
		{
			...
			pPointLightPixelCB->ShadowMapPixelSize = 1.0f / (float)m_iShadowMapSize;
			pPointLightPixelCB->LightSize = 25.0f;
		}
	...
	pd3dImmediateContext->GSSetShader(NULL, NULL, 0);
	pd3dImmediateContext->PSSetShader(bWireframe ? m_pDebugLightPixelShader : (iShadowmapIdx >= 0 ? (m_bEnablePCSS ? m_pSpotLightPCSShadowPixelShader : m_pSpotLightShadowPixelShader) : m_pSpotLightPixelShader), NULL, 0);
