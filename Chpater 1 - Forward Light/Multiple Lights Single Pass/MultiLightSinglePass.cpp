...
float g_fLightRange = 30.0f;
float g_fLightOuterAngle = 60.0f;
float g_fLightInnerAngle = 55.0f;
float g_fLightLength = 85.0f;
D3DXVECTOR3 g_arrLightColor[g_iNumLights];
int g_iActiveLights = g_iNumLights;
...
#define IDC_LIGHT_LENGTH			15
#define IDC_ACTIVE_LIGHTS			16
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    DXUTCreateWindow( L"Multiple Light in a Single Pass" );
...
void InitApp()
{
	...
	g_HUD.AddStatic( 0, L"Light Length:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_LIGHT_LENGTH, iSliderX, iY, iSliderW, 22, 0, 150, (int)(g_fLightLength));
	g_HUD.AddStatic( 0, L"Total Lights:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_ACTIVE_LIGHTS, iSliderX, iY, iSliderW, 22, 0, g_iNumLights, g_iActiveLights);
                                     ...
                                     void* pUserContext )
{
	...
	g_arrLightDirections[0] = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	g_arrLightDirections[1] = D3DXVECTOR3(0.0f, 0.0f ,0.0f) - g_arrLightPositions[1];
	D3DXVec3Normalize( &g_arrLightDirections[1], &g_arrLightDirections[1] );
	g_arrLightDirections[2] = g_arrLightPositions[1] - g_arrLightPositions[0];
	D3DXVec3Normalize( &g_arrLightDirections[2], &g_arrLightDirections[2] );
                                 ...
                                 float fElapsedTime, void* pUserContext )
{
	...
	// Do the directional light first 
	g_LightManager.SetupDirectional(pd3dImmediateContext);
	g_SceneManager.RenderScene(pd3dImmediateContext);

	// Perform the forward light calculations for all the lights in groups of four
	int iTotalLights = g_LightManager.GetNumLights();
	for(int i = 0; i < iTotalLights; i+=4)
	{
		g_LightManager.SetupLight(pd3dImmediateContext, i);
		g_SceneManager.RenderScene(pd3dImmediateContext);
	}
...
void CALLBACK OnFrameMove( double fTime, float fElapsedTime, void* pUserContext )
{
	...
	for(int i = 0; i < g_iActiveLights; i ++)
	{
		if(i == 0)
		{
		...
		{
			g_LightManager.AddSpotLight( g_arrLightPositions[i], g_arrLightDirections[i], g_fLightRange, g_fLightOuterAngle, g_fLightInnerAngle, g_arrLightColor[i]);
		}
		else if(i == 2)
		{
			g_LightManager.AddCapsuleLight( g_arrLightPositions[i] - 0.5f * g_fLightLength * g_arrLightDirections[i], g_arrLightDirections[i], g_fLightRange, g_fLightLength, g_arrLightColor[i]);
		}
	}
}
...
void CALLBACK OnMouse(bool bLeftButtonDown, bool bRightButtonDown, bool bMiddleButtonDown, bool bSideButton1Down, bool bSideButton2Down, int nMouseWheelDelta, int xPos, int yPos, void* pUserContext)
{
	...
	if(bRightButtonDown && (xPos != iPrevPosX || yPos != iPrevPosY))
	{
		...
		for(int i = 0; i < g_iNumLights; i ++)
		{
			...
			D3DXVec3TransformNormal( &g_arrLightDirections[i], &g_arrLightDirections[i], &rotationAroundY );
		}
	}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
		...
		case IDC_LIGHT_RANGE:
			g_fLightRange = fVal;
			break;
		case IDC_LIGHT_LENGTH:
			g_fLightLength = fVal;
			break;
		case IDC_ACTIVE_LIGHTS:
			g_iActiveLights = ((CDXUTSlider*)pControl)->GetValue();
			break;
    }
}
