...
public:
	...
	int GetNumLights() const { return m_arrLight.size(); }

	// Setup directional light
	void SetupDirectional(ID3D11DeviceContext* pd3dImmediateContext);

	// Setup four lights from the given light index
	void SetupLight(ID3D11DeviceContext* pd3dImmediateContext, int iLightIdx);
...
private:
	...
	// Setup four lights
	void SetupFour(ID3D11DeviceContext* pd3dImmediateContext, int iNumLights, const LIGHT* lights);
	...
	// Four light resources
	ID3D11PixelShader* m_pFourLightPixelShader;
	ID3D11Buffer* m_pFourLightPixelCB;
