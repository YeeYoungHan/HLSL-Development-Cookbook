...
CLightManager::CLightManager() : m_pForwardLightVertexShader(NULL), m_pForwardLightVSLayout(NULL),
	...
	m_pSpotLightPixelShader(NULL), m_pSpotLightPixelCB(NULL),
	m_pCapsuleLightPixelShader(NULL), m_pCapsuleLightPixelCB(NULL),
...
HRESULT CLightManager::Init()
{
	...
	cbDesc.ByteWidth = sizeof( CB_SPOT );
	V_RETURN( g_pDevice->CreateBuffer( &cbDesc, NULL, &m_pSpotLightPixelCB ) );
	DXUT_SetDebugName( m_pSpotLightPixelCB, "Spot Light CB" );
	
	cbDesc.ByteWidth = sizeof( CB_CAPSULE );
	V_RETURN( g_pDevice->CreateBuffer( &cbDesc, NULL, &m_pCapsuleLightPixelCB ) );
	DXUT_SetDebugName( m_pCapsuleLightPixelCB, "Capsule Light CB" );
...
void CLightManager::LightSetup(ID3D11DeviceContext* pd3dImmediateContext, int iLightIdx)
{
	if(iLightIdx == 0)
	{
		...
		SetupDirectional(pd3dImmediateContext);
	}
	else
	{
		if(iLightIdx == 1)
		...
		{
			SetupSpot(pd3dImmediateContext, light);
		}
		else if(light.eLightType == TYPE_CAPSULE)
		{
			SetupCapsule(pd3dImmediateContext, light);
		}
	}
}
...
void CLightManager::SetupPoint(ID3D11DeviceContext* pd3dImmediateContext, LIGHT& light)
{
	...
	pCB->vPosition = light.vPosition;
    pCB->fRangeRcp = 1.0f / light.fRange;
	pCB->vColor = GammaToLinear(light.vColor);
...
void CLightManager::SetupSpot(ID3D11DeviceContext* pd3dImmediateContext, LIGHT& light)
{
	HRESULT hr;
	...
	CB_SPOT* pCB = ( CB_SPOT* )MappedResource.pData;
	pCB->vPosition = light.vPosition;
	pCB->vDirToLight = -light.vDirection;
	pCB->fRangeRcp = 1.0f / light.fRange;
	pCB->fCosOuterCone = cosf(light.fOuterAngle);
	pCB->fCosInnerConeRcp = 1.0f / cosf(light.fInnerAngle);
	pCB->vColor = GammaToLinear(light.vColor);
	...
	// Set the shaders
	pd3dImmediateContext->VSSetShader(m_pForwardLightVertexShader, NULL, 0);
	pd3dImmediateContext->PSSetShader(m_pSpotLightPixelShader, NULL, 0);
}

void CLightManager::SetupCapsule(ID3D11DeviceContext* pd3dImmediateContext, LIGHT& light)
{
	HRESULT hr;
	...
	CB_CAPSULE* pCB = ( CB_CAPSULE* )MappedResource.pData;
	pCB->vPosition = light.vPosition;
	pCB->vDirection = light.vDirection;
	pCB->fLength = light.fLength;
	pCB->fRangeRcp = 1.0f / light.fRange;
	pCB->vColor = GammaToLinear(light.vColor);
	...
	// Set the shaders
	pd3dImmediateContext->VSSetShader(m_pForwardLightVertexShader, NULL, 0);
	pd3dImmediateContext->PSSetShader(m_pCapsuleLightPixelShader, NULL, 0);
}
