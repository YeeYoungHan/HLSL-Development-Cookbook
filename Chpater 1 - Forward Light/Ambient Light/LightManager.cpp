...
CLightManager::CLightManager() : m_pForwardLightVertexShader(NULL), m_pForwardLightVSLayout(NULL),
	m_pAmbientLightPixelShader(NULL), m_pAmbientLightPixelCB(NULL), m_pForwardLightDS(NULL)
{
}
...
HRESULT CLightManager::Init()
{
    ...
    V_RETURN( CompileShader(str, NULL, "AmbientLightPS", "ps_5_0", dwShaderFlags, &pShaderBlob) );
    V_RETURN( g_pDevice->CreatePixelShader( pShaderBlob->GetBufferPointer(),
                                             pShaderBlob->GetBufferSize(), NULL, &m_pAmbientLightPixelShader ) );
    DXUT_SetDebugName( m_pAmbientLightPixelShader, "Ambient Light PS" );
	SAFE_RELEASE( pShaderBlob );
...
void CLightManager::DeInit()
{
	...
	SAFE_RELEASE( m_pAmbientLightPixelShader );
	SAFE_RELEASE( m_pAmbientLightPixelCB );
