...
D3DXVECTOR3 g_vAmbientLowerColor = D3DXVECTOR3(0.5f, 0.5f, 0.5f);
D3DXVECTOR3 g_vAmbientUpperColor = D3DXVECTOR3(0.53f, 0.53f, 0.6f);
...
float g_fDecalSizeMax = 20.0f;
float g_fDecalSize = 8.5f;
bool g_bShowWireframe = false;
...
#define IDC_DECAL_WIRE				36
#define IDC_DECAL_RADIUS			37
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    DXUTCreateWindow( L"Dynamic Decals" );
...
void InitApp()
{
	...
	g_HUD.AddCheckBox( IDC_DECAL_WIRE, L"Decal Wireframe On / Off", 0, iY += iYo, 170, 22, g_bShowWireframe);
	g_HUD.AddStatic( 0, L"Decal Radius:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_DECAL_RADIUS, iSliderX, iY, iSliderW, 22, 0, 255, (int)((g_fDecalSize / g_fDecalSizeMax) * 255.0f));
                                     ...
                                     void* pUserContext )
{
	...
	D3DXVECTOR3 vecEye( 0.0f, 12.0f, 15.0f );
	D3DXVECTOR3 vecAt( 0.0f, 0.0f, 0.0f );
	D3DXVECTOR3 vMin = D3DXVECTOR3( -150.0f, -80.0f, -150.0f );
	D3DXVECTOR3 vMax = D3DXVECTOR3( 150.0f, 50.0f, 150.0f );
                                 ...
                                 float fElapsedTime, void* pUserContext )
{
	...
	g_DecalManager.PreRender(pd3dImmediateContext);
    ...
    float ClearColor[4] = { 0.1f, 0.11, 0.3, 0.0f };
	...
	// render the scene and the decals into the GBuffer
	g_GBuffer.PreRender(pd3dImmediateContext);
	g_SceneManager.RenderSceneToGBuffer(pd3dImmediateContext);
	g_DecalManager.Render(pd3dImmediateContext, g_bShowWireframe);
...
void CALLBACK OnFrameMove( double fTime, float fElapsedTime, void* pUserContext )
{
	...
	g_DecalManager.SetDecalScale(g_fDecalSize);
}
...
void CALLBACK OnMouse(bool bLeftButtonDown, bool bRightButtonDown, bool bMiddleButtonDown, bool bSideButton1Down, bool bSideButton2Down, int nMouseWheelDelta, int xPos, int yPos, void* pUserContext)
{
	if(bRightButtonDown && !g_bRightButtonDown)
	{
		...
		D3DXVECTOR3 vRayDir;
		vRayDir.x = v.x * viewInv._11 + v.y * viewInv._21 + v.z * viewInv._31;
		vRayDir.y = v.x * viewInv._12 + v.y * viewInv._22 + v.z * viewInv._32;
		vRayDir.z = v.x * viewInv._13 + v.y * viewInv._23 + v.z * viewInv._33;
		D3DXVECTOR3 vRayPos;
		vRayPos.x = viewInv._41;
		vRayPos.y = viewInv._42;
		vRayPos.z = viewInv._43;

		vRayPos = *g_Camera.GetEyePt();
		D3DXVec3Normalize(&vRayDir, &vRayDir);

		// Find the mesh intersected by the ray
		D3DXVECTOR3 vHitPos;
		D3DXVECTOR3 vHitNorm;
		CDXUTSDKMesh *pMesh;
		UINT nMeshIdx = 0;
		UINT nSubMeshIdx = 0;
		if(g_SceneManager.RayIntersection(vRayPos, vRayDir, vHitPos, vHitNorm, pMesh, nMeshIdx, nSubMeshIdx))
		{
			g_DecalManager.AddDecal(vHitPos, vHitNorm, *pMesh, nMeshIdx, nSubMeshIdx);
		}
	}
	else
	{
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
		...
		case IDC_SSAO_VISUALIZE:
			...
			break;
		case IDC_DECAL_WIRE:
			g_bShowWireframe = ((CDXUTCheckBox*)pControl)->GetChecked();
			break;
		case IDC_DECAL_RADIUS:
			g_fDecalSize = fNormVal * g_fDecalSizeMax;
			break;
    }
}
