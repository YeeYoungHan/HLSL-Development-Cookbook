...
D3DXVECTOR3 g_vDirLightColor = D3DXVECTOR3(0.75f, 0.74f, 0.7f);
float g_fTimeOfDay = 0.067f;
...
float g_fDOFFarRangeMax = 0.0f;
float g_fDOFFarRange = 70.0f;
...
float g_fMaxFogGlobalDensity = 2.0f;
...
#define IDC_TOD						6
#define IDC_FOG_COLOR_R				7
#define IDC_FOG_COLOR_G				8
#define IDC_FOG_COLOR_B				9
#define IDC_FOG_HL_COLOR_R			10
#define IDC_FOG_HL_COLOR_G			11
#define IDC_FOG_HL_COLOR_B			12
#define IDC_FOG_START				13
#define IDC_FOG_DENSITY				14
#define IDC_FOG_HEIGHT_FALLOFF		15
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    DXUTCreateWindow( L"Distance / Height Based Fog" );
...
void InitApp()
{
	...
	g_HUD.AddStatic( 0, L"Fog Color R:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_FOG_COLOR_R, iSliderX, iY, iSliderW, 22, 0, 255, (int)(g_vFogColor.x * 255.0f));
	g_HUD.AddStatic( 0, L"Fog Color G:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_FOG_COLOR_G, iSliderX, iY, iSliderW, 22, 0, 255, (int)(g_vFogColor.y * 255.0f));
	g_HUD.AddStatic( 0, L"Fog Color B:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_FOG_COLOR_B, iSliderX, iY, iSliderW, 22, 0, 255, (int)(g_vFogColor.z * 255.0f));
	g_HUD.AddStatic( 0, L"Fog Highlight Color R:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_FOG_HL_COLOR_R, iSliderX, iY, iSliderW, 22, 0, 255, (int)(g_vFogHighlightColor.x * 255.0f));
	g_HUD.AddStatic( 0, L"Fog Highlight Color G:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_FOG_HL_COLOR_G, iSliderX, iY, iSliderW, 22, 0, 255, (int)(g_vFogHighlightColor.y * 255.0f));
	g_HUD.AddStatic( 0, L"Fog  HighlightColor B:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_FOG_HL_COLOR_B, iSliderX, iY, iSliderW, 22, 0, 255, (int)(g_vFogHighlightColor.z * 255.0f));
	g_HUD.AddStatic( 0, L"Fog Start:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_FOG_START, iSliderX, iY, iSliderW, 22, 0, 200, (int)g_fFogStartDepth);
	g_HUD.AddStatic( 0, L"Fog Density:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_FOG_DENSITY, iSliderX, iY, iSliderW, 22, 0, 255, (int)((g_fFogGlobalDensity / g_fMaxFogGlobalDensity) * 255.0f));
	g_HUD.AddStatic( 0, L"Fog Height Falloff:", 0, iY += iYo, 170, 22);
	g_HUD.AddSlider( IDC_FOG_HEIGHT_FALLOFF, iSliderX, iY, iSliderW, 22, 0, 255, (int)(g_fFogHeightFalloff * 255.0f));
                                         ...
                                         const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
	...
	V_RETURN( g_pDevice->CreateBuffer( &cbDesc, NULL, &g_pFogCB ) );
	DXUT_SetDebugName( g_pFogCB, "Fog CB" );
                                 ...
                                 float fElapsedTime, void* pUserContext )
{
	...
	pd3dImmediateContext->Map( g_pFogCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedResource);
	CB_FOG_PS* pCB = ( CB_FOG_PS* )MappedResource.pData;
	pCB->vColor = g_vFogColor;
	pCB->vHighlightColor = g_vFogHighlightColor;
	pCB->fStartDepth = g_fFogStartDepth;
	pCB->fGlobalDensity = g_fFogGlobalDensity;
	pCB->vDirToSun = -g_vDirLightDir;
	pCB->fHeightFalloff = g_fFogHeightFalloff;
	pd3dImmediateContext->Unmap( g_pFogCB, 0 );
	pd3dImmediateContext->PSSetConstantBuffers( 2, 1, &g_pFogCB );

	float fHighlightColorFactor = -D3DXVec3Dot(g_Camera.GetWorldAhead(), &g_vDirLightDir) * 1.5f * abs(g_fTimeOfDay - 0.5f);
	D3DXVECTOR3 vClearColor = g_vFogHighlightColor * fHighlightColorFactor + g_vFogColor * (1.0f - fHighlightColorFactor);
    float ClearColor[4] = { vClearColor.x, vClearColor.y, vClearColor.z, 0.0f };
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
		...
		case IDC_TOD:
			...
			break;
		case IDC_FOG_COLOR_R:
			g_vFogColor.x = fNormVal;
			break;
		case IDC_FOG_COLOR_G:
			g_vFogColor.y = fNormVal;
			break;
		case IDC_FOG_COLOR_B:
			g_vFogColor.z = fNormVal;
			break;
		case IDC_FOG_HL_COLOR_R:
			g_vFogHighlightColor.x = fNormVal;
			break;
		case IDC_FOG_HL_COLOR_G:
			g_vFogHighlightColor.y = fNormVal;
			break;
		case IDC_FOG_HL_COLOR_B:
			g_vFogHighlightColor.z = fNormVal;
			break;
		case IDC_FOG_START:
			g_fFogStartDepth = fVal;
			break;
		case IDC_FOG_DENSITY:
			g_fFogGlobalDensity = fNormVal * g_fMaxFogGlobalDensity;
			break;
		case IDC_FOG_HEIGHT_FALLOFF:
			g_fFogHeightFalloff = fNormVal;
			break;
    }
}
