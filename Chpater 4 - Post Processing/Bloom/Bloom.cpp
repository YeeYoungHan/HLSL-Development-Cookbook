...
float g_fBloomThresholdMax = 2.5f;
float g_fBloomThreshold = 1.1f;
                                     ...
                                     void* pUserContext )
{
	...
	// Update the direction for the first time so we can set the camera
                                 ...
                                 float fElapsedTime, void* pUserContext )
{
	...
	if(g_bEnablePostFX)
	{
		...
		g_PostFX.PostProcessing(pd3dImmediateContext, m_HDRSRV, DXUTGetD3D11RenderTargetView());
