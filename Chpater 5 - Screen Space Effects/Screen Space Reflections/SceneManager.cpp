...
CSceneManager::CSceneManager() : m_fSunRadius(15.0f),
...
HRESULT CSceneManager::Init()
{
	...
	V_RETURN( m_MeshOpaque.Create( g_pDevice, L"..\\Media\\ScreenSpaceReflect\\room.sdkmesh" ) );
	V_RETURN( m_MeshReflection.Create( g_pDevice, L"..\\Media\\ScreenSpaceReflect\\sphere.sdkmesh" ) );
...
void CSceneManager::RenderSceneToGBuffer(ID3D11DeviceContext* pd3dImmediateContext)
{
	...
	m_MeshReflection.Render(pd3dImmediateContext, 0, 1);
}
...
void CSceneManager::RenderSceneNoShaders(ID3D11DeviceContext* pd3dImmediateContext)
{
	...
	m_MeshReflection.Render(pd3dImmediateContext);
}
...
void CSceneManager::RenderReflection(ID3D11DeviceContext* pd3dImmediateContext)
{
	m_MeshReflection.Render(pd3dImmediateContext);
}
