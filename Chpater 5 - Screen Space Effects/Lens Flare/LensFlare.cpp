                                 ...
                                 float fElapsedTime, void* pUserContext )
{
	...
	// Add the lens flares just before the sky
	g_LensflareManager.Render(pd3dImmediateContext);

	// Render target and render the sky
	g_SceneManager.RenderSky(pd3dImmediateContext, g_vDirLightDir, 2.0f * g_vDirLightColor);
...
void CALLBACK OnFrameMove( double fTime, float fElapsedTime, void* pUserContext )
{
	...
	D3DXVECTOR3 vSunPosition = D3DXVECTOR3(vEyePos.x - 200.0f * g_vDirLightDir.x, -200.0f * g_vDirLightDir.y, vEyePos.z - 200.0f * g_vDirLightDir.z);
	g_LensflareManager.Update(vSunPosition, fElapsedTime);
}
