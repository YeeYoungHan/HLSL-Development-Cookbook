...
public:
	...
	HRESULT Init();
	void Deinit();
	void Update(const D3DXVECTOR3& sunWorldPos, float fElapsedTime);
	...
	static const int m_TotalCoronaFlares = 4;
	static const int m_TotalFlares = 10;
...
private:
	...
	ID3D11BlendState* m_pNoColorBlendState;
	...
	float m_fCoronaRotation;

	// Store pre-visibility blend values
	ID3D11BlendState* m_pPrevBlendState;
	FLOAT m_prevBlendFactor[ 4 ];
	UINT m_prevSampleMask;
};