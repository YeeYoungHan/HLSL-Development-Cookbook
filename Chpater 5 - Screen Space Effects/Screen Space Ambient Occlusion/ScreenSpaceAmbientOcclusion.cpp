...
ID3D11ShaderResourceView* g_pWhiteTexSRV = NULL;
...
int g_iSSAOSampRadiusMax = 20;
int g_iSSAOSampRadius = 10;
float g_fSSAORadiusMax = 50.0f;
...
#define IDC_SSAO_SAMP_RADIUS		6
#define IDC_SSAO_RADIUS				7
#define IDC_SSAO_VISUALIZE			8
#define IDC_SSAO_ENABLE				9
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    DXUTCreateWindow( L"Screen Space Ambient Occlusion (SSAO)" );
...
void InitApp()
{
	...
	g_HUD.AddCheckBox( IDC_SSAO_ENABLE, L"Enable SSAO", 0, iY += iYo, 170, 22, g_bEnableSSAO);
                                         ...
                                         const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
	...
	SAFE_RELEASE( g_HDRRTV );
	SAFE_RELEASE( g_HDRSRV );
	SAFE_RELEASE( g_pWhiteTexSRV );
	...
	V_RETURN( g_pDevice->CreateTexture2D( &dtd, NULL, &g_pHDRTexture ) );
	DXUT_SetDebugName( g_pHDRTexture, "HDR Light Accumulation Texture" );
	...
	V_RETURN( g_pDevice->CreateRenderTargetView( g_pHDRTexture, &rtsvd, &g_HDRRTV ) ); 
	DXUT_SetDebugName( g_HDRRTV, "HDR Light Accumulation RTV" );
	...
	V_RETURN( g_pDevice->CreateShaderResourceView( g_pHDRTexture, &dsrvd, &g_HDRSRV ) );
	DXUT_SetDebugName( g_HDRSRV, "HDR Light Accumulation SRV" );

	WCHAR str[MAX_PATH];
	V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"..\\Media\\white.dds" ) );
	V_RETURN( D3DX11CreateShaderResourceViewFromFile( g_pDevice, str, NULL, NULL, &g_pWhiteTexSRV, NULL ) );
	DXUT_SetDebugName( g_pWhiteTexSRV, "SSAO Empty Texture SRV" );
                                 ...
                                 float fElapsedTime, void* pUserContext )
{
	...
	pd3dImmediateContext->ClearRenderTargetView( g_bEnablePostFX ? g_HDRRTV : pRTV, ClearColor );
	...
	{
		// Sampling a white texture is equivalent to no occlusion
		arrSRV[0] = g_pWhiteTexSRV;
	}
	pd3dImmediateContext->PSSetShaderResources(5, 1, arrSRV);

	// Set the HDR render target and do the lighting
	pd3dImmediateContext->OMSetRenderTargets( 1, g_bEnablePostFX ? &g_HDRRTV : &pRTV, g_GBuffer.GetDepthReadOnlyDSV() );
	...
	if(g_bEnablePostFX)
	{
		...
		g_PostFX.PostProcessing(pd3dImmediateContext, g_HDRSRV, g_GBuffer.GetDepthView(), DXUTGetD3D11RenderTargetView());
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
		...
		case IDC_SSAO_VISUALIZE:
			...
			break;
		case IDC_SSAO_ENABLE:
			g_bEnableSSAO = ((CDXUTCheckBox*)pControl)->GetChecked();
			break;
    }
}
