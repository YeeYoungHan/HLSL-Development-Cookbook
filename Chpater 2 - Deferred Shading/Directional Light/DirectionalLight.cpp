...
D3DXVECTOR3 g_vDirLightColor = D3DXVECTOR3(0.85f, 0.8f, 0.5f);
...
#define IDC_LOWER_COLOR_R			6
#define IDC_LOWER_COLOR_G			7
#define IDC_LOWER_COLOR_B			8
#define IDC_UPPER_COLOR_R			9
#define IDC_UPPER_COLOR_G			10
#define IDC_UPPER_COLOR_B			11
#define IDC_DIRLIGHT_COLOR_R		12
#define IDC_DIRLIGHT_COLOR_G		13
#define IDC_DIRLIGHT_COLOR_B		14
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    DXUTCreateWindow( L"Directional Light" );
...
void CALLBACK OnMouse(bool bLeftButtonDown, bool bRightButtonDown, bool bMiddleButtonDown, bool bSideButton1Down, bool bSideButton2Down, int nMouseWheelDelta, int xPos, int yPos, void* pUserContext)
{
	...
	if(bRightButtonDown && (xPos != iPrevPosX || yPos != iPrevPosY))
	{
		...
		D3DXMatrixInverse( &viewInvMat, NULL, g_Camera.GetViewMatrix() );
		...
		g_vDirLightDir -= vRight * fDX;
		g_vDirLightDir -= vUp * fDY;
		g_vDirLightDir += vForward * fDY;

		D3DXVec3Normalize( &g_vDirLightDir, &g_vDirLightDir );
	}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
	...
	if(pControl->GetType() == DXUT_CONTROL_SLIDER)
	{
		...
		fNormVal = (float)iCurVal / (float)(iRangeMax - iRangeMin);
	}
